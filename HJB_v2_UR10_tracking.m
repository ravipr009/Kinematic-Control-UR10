...
I = [1 0 0;0 1 0;0 0 1];
theta0=rand(6,1);
% t0=a2*cos(theta0(2,1))+a3*cos(theta0(2,1)+theta0(3,1));
% x0=cos(theta0(1,1))*t0-d3*sin(theta0(1,1));
% y0=sin(theta0(1,1))*t0+d3*cos(theta0(1,1));
% z0=-a2*sin(theta0(2,1))-a3*sin(theta0(2,1)+theta0(3,1));
...
%          up=num/(den)+ J'*w(:,k); % mine with steady state Jinv* rd dot RNN estimate
                  up=num/(den)+ Ji*((rd(:,k+1)-rd(:,k))/dt); % mine with steady state Jinv* rd dot SVF estimate
...
grid on;
title(' Reference Trajectory v/s Actual Trajectory')
xlabel('X-axis')
ylabel('Y-axis')
zlabel('Z-axis')
